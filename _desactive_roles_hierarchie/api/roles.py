...
dossier. Ce router n'est enregistré dans aucun app.include_router(...) de
api/main.py depuis avant cette date -- aucun endpoint ici n'est joignable.

Les fonctions encore réellement utilisées par le reste de l'app
(messagerie directe de l'outil IA envoyer_message) ont été extraites vers
core/messagerie_directe.py le 06/09/2026 -- ce fichier ne fait plus
qu'importer depuis là, ne les redéfinit plus.
...
from core.messagerie_directe import (
    _nom_affiche_ou_repli,
    _peut_echanger_messages,
    _contacts_autorises,
    _inserer_message,
)
